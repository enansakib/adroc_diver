...
class ADROC_Action:
    ...
    def __init__(self, name):
        ...
        rospy.Subscriber('/adroc_diver/yaw_angle', Float64, self.angle_cb)
        self.drp_msgs = list()
        self.last_drp_msg = 0
        ...
        self.model = 'knn' ## ['nn-knn', nn-svm', 'nn', 'knn', 'svm']
        self.n_neighbors = 5
        if self.data_type == 'all':
            ...
            self.knn_path = '/home/irvlab/adroc_diver_id_ws/src/adroc_diver/scripts/clf_knndiver'
            self.svm_path = '/home/irvlab/adroc_diver_id_ws/src/adroc_diver/scripts/clf_svmdiver'
        ...
        self.batch_size = 256
        self.TRAINING_DONE = True ## Plan A and False for plan B
    ...
    def angle_cb(self, msg):
        ...
        rospy.loginfo("yaw_angle in cb: %f",self.yaw_angle)
        if self.yaw_angle > 180:
            self.yaw_angle = self.yaw_angle - 360

        rospy.loginfo("yaw_angle after sub: %f", self.yaw_angle)
    ...
    def drp_stable(self):
        if len(self.drp_msgs) >0:
            ...
            image_setpoint_x = self.drp_msgs[0].image_w/2
            # image_setpoint_y = self.drp_msgs[0].image_h/2.0 #########
            image_setpoint_y = self.drp_msgs[0].image_h/4 #########
    ...
    def perform_AngleYaw(self):
        ...
        while self.target_yaw_angle < self.yaw_angle:
            self.cmd_pub.publish(msg)
            rospy.loginfo("ADROC is performing a Angle yaw at %f", msg.yaw)
    ...
    def perform_TimedYaw(self, counter):
        ...
        self.search_it_count = 0
        # time.sleep(1) ## we add this for testing on bench, to have time between switching people
        return
    ...
    def run(self):
        if self.state == ADROCState.INIT:
            ...
            # self.perform_TimedYaw(5) ## TUNE this argument, increasing will make it more yaw
            self.perform_AngleYaw()
