...
class DRP_Processor:
    def __init__(self):
        ...
        if self.visualize:
            ...
            rospy.Subscriber('/adroc_diver/state', String, self.state_cb)
        ...
        self.ls_conf = 0.0
        self.ls_ts = 0
        self.rh_observation = [0,0]
        self.rh_conf = 1.0
        self.rh_ts = 0
        self.lh_observation = [0,0]
        self.lh_conf = 0.0
        self.lh_ts = 0
        ...
        # self.bbox_target_ratio = 0.17 * 1.5 * 2 * 1.2/0.7 * 8/9 ###### 0.7 for non-square , 0.7 ** 2.3 (2.5 square) worked
        # self.shoulder_target_ratio= 0.17 ######## 0.17
        self.bbox_target_ratio = 0.17 * 1.5 * 2 * 1.2/0.7 * 8/9 / 1.6 ###### 0.7 for non-square , 0.7 ** 2.3 (2.5 square) worked
        self.shoulder_target_ratio= 0.17 /2.1 ######## 0.17
    ...
    def pose_msg_cb(self, msg):
        ...
        self.rh_observation = [round(observations[6]), round(observations[7])]
        # self.rs_conf = msg.right_shoulder.confidence
        self.rh_ts = rospy.Time.now().to_sec()

        self.lh_observation =  [round(observations[4]), round(observations[5])]
        # self.ls_conf = msg.left_shoulder.confidence
        self.lh_ts = rospy.Time.now().to_sec()
    ...
    def draw_drp_image(self, centerpoint, pseudo_distance, draw_pose, draw_bbox):
        ...
        cv2.putText(self.cv_image , "Current State : "+self.state, (self.image_w - 500, self.image_h-40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
        ...
        if draw_pose:
            if self.ls_observation:
                ...
                cv2.putText(self.cv_image , "LS", (x-10, y-20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
            if self.rs_observation:
                x, y = self.rs_observation 
                cv2.circle(self.cv_image, (x,y), 7, (0, 0, 255),3)
                cv2.putText(self.cv_image , "RS", (x-10, y-20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
            if self.lh_observation:
                x, y = self.lh_observation 
                cv2.circle(self.cv_image, (x,y), 7, (255, 0, 0),3)
                cv2.putText(self.cv_image , "LH", (x-10, y-20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
            if self.rh_observation:
                x, y = self.rh_observation 
                cv2.circle(self.cv_image, (x,y), 7, (0, 0, 255),3)
                cv2.putText(self.cv_image , "RH", (x-10, y-20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
    ...
    def process(self):
        if self.drp_active:
            ...
            if (not cp_pose is None) and (not cp_bbox is None): #Both are available.
                ...
                image = self.bridge_object.cv2_to_imgmsg(self.cv_image, "bgr8")
            ...
            ## updated to visualize when drp is not active
            if self.visualize:
                image = self.bridge_object.cv2_to_imgmsg(self.cv_image, "bgr8")
                self.drp_image_pub.publish(image)
            else:
